window.onload = function() {
    ...
    function create() {
        ...
        cardBack = game.add.sprite(game.world.width-168,game.world.height/2-178,'cardBack');
        cardBack.inputEnabled = true;
        cardBack.events.onInputDown.add(drawCard,this);
        
        startCard = deck.getRandom();
        startCard.reset(game.world.width/2-84,game.world.height/2-178);
        startCard.inDeck = false;
        startCard.middle = true;
        
        c1 = deck.getRandom();
        c1.reset(game.world.width/2-180,game.world.height/2+156)
        c1.inDeck = false;
        c1.inHand = true;
        
        c2 = deck.getRandom();
        c2.reset(game.world.width/2-84,game.world.height/2+156)
        c2.inDeck = false;
        c2.inHand = true;
        
        c3 = deck.getRandom();
        c3.reset(game.world.width/2+12,game.world.height/2+156)
        c3.inDeck = false;
        c3.inHand = true;
        
        startSpot = game.add.sprite(startCard.x-84,startCard.y,'startSpot');
        ...
        startSpot = game.add.sprite(startCard.x,startCard.y-130,'startSpot');
        startSpot.inputEnabled = true;
        startSpot.cL = false;
        startSpot.cR = false;
        startSpot.cU = true;
    ...
    function checkCrash(){
        if (cL.cVal === cR.cVal && cL.black === cR.black){
            ...
            if (cU.cVal === cD.cVal && cU.black === cD.black){
                crash();
            }
        }
    }
